    ...
    if (!Vue.prototype.$storyapi) {
      ...
      Vue.prototype.$storybridge = {
        ...
        on: function(events, cb, options) {
          ...
          this.load(() => {
            if ('<%= options.scriptVersion %>' === 'v2') {
              window.storyblok = new StoryblokBridge(options)
        ...
        load: function(cb, errorCb) {
          ...
          loadScript('https://app.storyblok.com/f/storyblok-' +
          '<%= options.scriptVersion %>' === 'v2' ? 'v2-' : '' + 'latest.js', () => {
